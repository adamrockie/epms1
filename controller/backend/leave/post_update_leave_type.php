...
if($user->isLoggedIn() && Token::check(Input::get('token'))){
    ...
    $id       = Sanitize::sanitize(Input::get('leave_id'));

    $saved = LeaveTypes::where('id', '=', $id)->update([
        'name'     => $name,
        'days'     => $days,
        'level'    => $category
    ]);
    ...
    if($saved){
        [$status, $message] = ['success', 'Leave details updated successfully'];
        echo json_encode(['status'=>$status, 'msg'=>$message]);
    }else{
        [$status, $message] = ['error', 'An error occurred, Leave details could not be updated'];
        echo json_encode(['status'=>$status, 'msg'=>$message]);
    }
}
